...
/*
 ...
 - efficient runtime (NB- we do allocate when "growing" content - creating a window / opening a tree Sprite for the first time, etc. - but a typical frame won't allocate anything)
 ...
 - 2016/05/02 (1.49) - renamed SetNextTreeSpriteOpened() to SetNextTreeSpriteOpen(), no redirection.
 - 2016/05/01 (1.49) - obsoleted old signature of CollapsingHeader(const char* label, const char* str_id = NULL, bool display_frame = true, bool default_open = false) as extra parameters were badly designed and rarely used. You can replace the "default_open = true" flag in new API with CollapsingHeader(label, ImGuiTreeSpriteFlags_DefaultOpen).
 ...
 - 2015/03/15 (1.36) - renamed style.TreeSpriteSpacing to style.IndentSpacing, ImGuiStyleVar_TreeSpriteSpacing to ImGuiStyleVar_IndentSpacing
 - 2015/03/13 (1.36) - renamed GetWindowIsFocused() to IsWindowFocused(). Kept inline redirection function until 1.50.
 - 2015/03/08 (1.35) - renamed style.ScrollBarWidth to style.ScrollbarWidth (casing)
 - 2015/02/27 (1.34) - renamed OpenNextSprite(bool) to SetNextTreeSpriteOpened(bool, ImGuiSetCond). Kept inline redirection function until 1.50.
 ...
 A: Yes. A primer on the use of labels/IDs in ImGui..
   ...
   - ID are uniquely scoped within windows, tree Sprites, etc. so no conflict can happen if you have two buttons called "OK" in two different windows
   ...
   - More example showing that you can stack multiple prefixes into the ID stack:
       ...
       PushID("Sprite");
       Button("Click");     // Label = "Click",  ID = hash of "Sprite" + "Click"
         PushID(my_ptr);
           Button("Click"); // Label = "Click",  ID = hash of "Sprite" + ptr + "Click"
         PopID();
       PopID();

   - Tree Sprites implicitly creates a scope for you by calling PushID().
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree Sprite.
     Depending on your use cases you may want to use strings, indices or pointers as ID.
      e.g. when displaying a single object that may change over time (1-1 relationship), using a static string as ID will preserve your Sprite open/closed state when the targeted object change.
      e.g. when displaying a list of objects, using indices or pointers as ID will preserve the Sprite open/closed state differently. experiment and see what makes more sense!
...
!- main: make it so that a frame with no window registered won't refocus every window on subsequent frames (~bump LastFrameActive of all windows).
 - main: IsItemHovered() make it more consistent for various type of widgets, widgets with multiple components, etc. also effectively IsHovered() region sometimes differs from hot region, e.g tree Sprites
 - main: IsItemHovered() info stored in a stack? so that 'if TreeSprite() { Text; TreePop; } if IsHovered' return the hover state of the TreeSprite?
...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - tree Sprite / optimization: avoid formatting when clipped.
 - tree Sprite: tree-Sprite/header right-most side doesn't take account of horizontal scrolling.
 - tree Sprite: add treeSprite/treepush int variants? not there because (void*) cast from int warns on some platforms/settings?
 - tree Sprite: try to apply scrolling at time of TreePop() if Sprite was just opened and end of Sprite is past scrolling limits?
 - tree Sprite / selectable render mismatch which is visible if you use them both next to each other (e.g. cf. property viewer)
 - tree Sprite: tweak color scheme to distinguish headers from selected tree Sprite (#581)
 ...
 - log: have more control over the log scope (e.g. stop logging when leaving current tree Sprite scope)
 - log: be able to log anything (e.g. right-click on a window/tree-Sprite, shows context menu? log into tty/file/clipboard)
 - log: let user copy any window content to clipboard easily (CTRL+C on windows? while moving it? context menu?). code is commented because it fails with multiple Begin/End pairs.
 - filters: set a current filter that tree Sprite can automatically query to hide themselves
...
ImGuiStyle::ImGuiStyle()
{
    ...
    IndentSpacing           = 21.0f;            // Horizontal spacing when e.g. entering a tree Sprite. Generally == (FontSize + FramePadding.x*2).
...
bool ImGui::TreeSpriteBehaviorIsOpen(ImGuiID id, ImGuiTreeSpriteFlags flags)
{
    if (flags & ImGuiTreeSpriteFlags_Leaf)
        return true;

    // We only write to the tree storage if the user clicks (or explicitely use SetNextTreeSprite*** functions)
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ImGuiStorage* storage = window->DC.StateStorage;

    bool is_open;
    if (g.SetNextTreeSpriteOpenCond != 0)
    {
        if (g.SetNextTreeSpriteOpenCond & ImGuiSetCond_Always)
        {
            is_open = g.SetNextTreeSpriteOpenVal;
            storage->SetInt(id, is_open);
        }
        else
        {
            // We treat ImGuiSetCondition_Once and ImGuiSetCondition_FirstUseEver the same because tree Sprite state are not saved persistently.
        ...
        g.SetNextTreeSpriteOpenCond = 0;
    }
    else
    {
        is_open = storage->GetInt(id, (flags & ImGuiTreeSpriteFlags_DefaultOpen) ? 1 : 0) != 0;
    }

    // When logging is enabled, we automatically expand tree Sprites (but *NOT* collapsing headers.. seems like sensible behavior).
    // NB- If we are above max depth we still allow manually opened Sprites to be logged.
    if (g.LogEnabled && !(flags & ImGuiTreeSpriteFlags_NoAutoOpenOnLog) && window->DC.TreeDepth < g.LogAutoExpandMaxDepth)
        is_open = true;

    return is_open;
}

bool ImGui::TreeSpriteBehavior(ImGuiID id, ImGuiTreeSpriteFlags flags, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const bool display_frame = (flags & ImGuiTreeSpriteFlags_Framed) != 0;
    ...
    // For regular tree Sprites, we arbitrary allow to click past 2 worth of ItemSpacing
    ...
    bool is_open = TreeSpriteBehaviorIsOpen(id, flags);
    if (!ItemAdd(interact_bb, &id))
    {
        if (is_open && !(flags & ImGuiTreeSpriteFlags_NoTreePushOnOpen))
            TreePushRawID(id);
        return is_open;
    }
    ...
    ImGuiButtonFlags button_flags = ImGuiButtonFlags_NoKeyModifiers | ((flags & ImGuiTreeSpriteFlags_AllowOverlapMode) ? ImGuiButtonFlags_AllowOverlapMode : 0);
    if (flags & ImGuiTreeSpriteFlags_OpenOnDoubleClick)
        button_flags |= ImGuiButtonFlags_PressedOnDoubleClick | ((flags & ImGuiTreeSpriteFlags_OpenOnArrow) ? ImGuiButtonFlags_PressedOnClickRelease : 0);
    bool hovered, held, pressed = ButtonBehavior(interact_bb, id, &hovered, &held, button_flags);
    if (pressed && !(flags & ImGuiTreeSpriteFlags_Leaf))
    {
        bool toggled = !(flags & (ImGuiTreeSpriteFlags_OpenOnArrow | ImGuiTreeSpriteFlags_OpenOnDoubleClick));
        if (flags & ImGuiTreeSpriteFlags_OpenOnArrow)
    ...
    if (flags & ImGuiTreeSpriteFlags_AllowOverlapMode)
        SetItemAllowOverlap();
    ...
    {
        // Unframed typed for tree Sprites
        if (hovered || (flags & ImGuiTreeSpriteFlags_Selected))
            RenderFrame(bb.Min, bb.Max, col, false);

        if (flags & ImGuiTreeSpriteFlags_Bullet)
            RenderBullet(bb.Min + ImVec2(text_offset_x * 0.5f, g.FontSize*0.50f + text_base_offset_y));
        else if (!(flags & ImGuiTreeSpriteFlags_Leaf))
...
// CollapsingHeader returns true when opened but do not indent nor push into the ID stack (because of the ImGuiTreeSpriteFlags_NoTreePushOnOpen flag).
// This is basically the same as calling TreeSpriteEx(label, ImGuiTreeSpriteFlags_CollapsingHeader | ImGuiTreeSpriteFlags_NoTreePushOnOpen). You can remove the _NoTreePushOnOpen flag if you want behavior closer to normal TreeSprite().
bool ImGui::CollapsingHeader(const char* label, ImGuiTreeSpriteFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeSpriteBehavior(window->GetID(label), flags | ImGuiTreeSpriteFlags_CollapsingHeader | ImGuiTreeSpriteFlags_NoTreePushOnOpen, label);
}

bool ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeSpriteFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    bool is_open = TreeSpriteBehavior(id, flags | ImGuiTreeSpriteFlags_CollapsingHeader | ImGuiTreeSpriteFlags_NoTreePushOnOpen | (p_open ? ImGuiTreeSpriteFlags_AllowOverlapMode : 0), label);
    if (p_open)
    {
...
bool ImGui::TreeSpriteEx(const char* label, ImGuiTreeSpriteFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeSpriteBehavior(window->GetID(label), flags, label, NULL);
}

bool ImGui::TreeSpriteExV(const char* str_id, ImGuiTreeSpriteFlags flags, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    return TreeSpriteBehavior(window->GetID(str_id), flags, g.TempBuffer, label_end);
}

bool ImGui::TreeSpriteExV(const void* ptr_id, ImGuiTreeSpriteFlags flags, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const char* label_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    return TreeSpriteBehavior(window->GetID(ptr_id), flags, g.TempBuffer, label_end);
}

bool ImGui::TreeSpriteV(const char* str_id, const char* fmt, va_list args)
{
    return TreeSpriteExV(str_id, 0, fmt, args);
}

bool ImGui::TreeSpriteV(const void* ptr_id, const char* fmt, va_list args)
{
    return TreeSpriteExV(ptr_id, 0, fmt, args);
}

bool ImGui::TreeSpriteEx(const char* str_id, ImGuiTreeSpriteFlags flags, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool is_open = TreeSpriteExV(str_id, flags, fmt, args);
    va_end(args);
    return is_open;
}

bool ImGui::TreeSpriteEx(const void* ptr_id, ImGuiTreeSpriteFlags flags, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool is_open = TreeSpriteExV(ptr_id, flags, fmt, args);
    va_end(args);
    return is_open;
}

bool ImGui::TreeSprite(const char* str_id, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool is_open = TreeSpriteExV(str_id, 0, fmt, args);
    va_end(args);
    return is_open;
}

bool ImGui::TreeSprite(const void* ptr_id, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool is_open = TreeSpriteExV(ptr_id, 0, fmt, args);
    va_end(args);
    return is_open;
}

bool ImGui::TreeSprite(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    return TreeSpriteBehavior(window->GetID(label), 0, label, NULL);
}

void ImGui::TreeAdvanceToLabelPos()
{
    ImGuiContext& g = *GImGui;
    g.CurrentWindow->DC.CursorPos.x += GetTreeSpriteToLabelSpacing();
}

// Horizontal distance preceding label when using TreeSprite() or Bullet()
float ImGui::GetTreeSpriteToLabelSpacing()
{
    ImGuiContext& g = *GImGui;
    return g.FontSize + (g.Style.FramePadding.x * 2.0f);
}

void ImGui::SetNextTreeSpriteOpen(bool is_open, ImGuiSetCond cond)
{
    ImGuiContext& g = *GImGui;
    g.SetNextTreeSpriteOpenVal = is_open;
    g.SetNextTreeSpriteOpenCond = cond ? cond : ImGuiSetCond_Always;
}
...
// Text with a little bullet aligned to the typical tree Sprite.
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            static void SpriteDrawList(ImDrawList* draw_list, const char* label)
            {
                bool Sprite_open = ImGui::TreeSprite(draw_list, "%s: '%s' %d vtx, %d indices, %d cmds", label, draw_list->_OwnerName ? draw_list->_OwnerName : "", draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
                if (draw_list == ImGui::GetWindowDrawList())
                {
                    ImGui::SameLine();
                    ImGui::TextColored(ImColor(255,100,100), "CURRENTLY APPENDING"); // Can't display stats for active draw list! (we don't have the data double-buffered)
                    if (Sprite_open) ImGui::TreePop();
                    return;
                }
                if (!Sprite_open)
                    return;
                ...
                for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
                {
                    ...
                    bool pcmd_Sprite_open = ImGui::TreeSprite((void*)(pcmd - draw_list->CmdBuffer.begin()), "Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
            ...
            static void SpriteWindows(ImVector<ImGuiWindow*>& windows, const char* label)
            {
                if (!ImGui::TreeSprite(label, "%s (%d)", label, windows.Size))
                    return;
                for (int i = 0; i < windows.Size; i++)
                    Funcs::SpriteWindow(windows[i], "Window");
                ImGui::TreePop();
            }

            static void SpriteWindow(ImGuiWindow* window, const char* label)
            {
                if (!ImGui::TreeSprite(window, "%s '%s', %d @ 0x%p", label, window->Name, window->Active || window->WasActive, window))
                    return;
                SpriteDrawList(window->DrawList, "DrawList");
                ...
                if (window->RootWindow != window) SpriteWindow(window->RootWindow, "RootWindow");
                if (window->DC.ChildWindows.Size > 0) SpriteWindows(window->DC.ChildWindows, "ChildWindows");
        ...
        Funcs::SpriteWindows(g.Windows, "Windows");
        if (ImGui::TreeSprite("DrawList", "Active DrawLists (%d)", g.RenderDrawLists[0].Size))
        {
            for (int i = 0; i < g.RenderDrawLists[0].Size; i++)
                Funcs::SpriteDrawList(g.RenderDrawLists[0][i], "DrawList");
            ImGui::TreePop();
        }
        if (ImGui::TreeSprite("Popups", "Open Popups Stack (%d)", g.OpenPopupStack.Size))
        {
        ...
        if (ImGui::TreeSprite("Basic state"))
        {
