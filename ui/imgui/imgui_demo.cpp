...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        if (ImGui::TreeSprite("Style"))
        {
            ImGui::ShowStyleEditor();
            ImGui::TreePop();
        }

        if (ImGui::TreeSprite("Logging"))
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a window or a block. Tree Sprites can be automatically expanded. You can also call ImGui::LogText() to output directly to the log without a visual output.");
            ImGui::LogButtons();
            ImGui::TreePop();
        }
    }
    ...
    {
        if (ImGui::TreeSprite("Trees"))
        {
            if (ImGui::TreeSprite("Basic trees"))
            {
                for (int i = 0; i < 5; i++)
                    if (ImGui::TreeSprite((void*)(intptr_t)i, "Child %d", i))
                    {
            ...
            if (ImGui::TreeSprite("Advanced, with Selectable Sprites"))
            {
                ShowHelpMarker("This is a more standard looking tree with selectable Sprites.\nClick to select, CTRL+Click to toggle, click on arrows or double-click to open.");
                static bool align_label_with_current_x_position = false;
                ImGui::Checkbox("Align label with current X position)", &align_label_with_current_x_position);
                ImGui::Text("Hello!");
                if (align_label_with_current_x_position)
                    ImGui::Unindent(ImGui::GetTreeSpriteToLabelSpacing());

                static int selection_mask = (1 << 2); // Dumb representation of what may be user-side selection state. You may carry selection state inside or outside your objects in whatever format you see fit.
                int Sprite_clicked = -1;                // Temporary storage of what Sprite we have clicked to process selection at the end of the loop. May be a pointer to your own Sprite type, etc.
                ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing, ImGui::GetFontSize()*3); // Increase spacing to differentiate leaves from expanded contents.
                for (int i = 0; i < 6; i++)
                {
                    // Disable the default open on single-click behavior and pass in Selected flag according to our selection state.
                    ImGuiTreeSpriteFlags Sprite_flags = ImGuiTreeSpriteFlags_OpenOnArrow | ImGuiTreeSpriteFlags_OpenOnDoubleClick | ((selection_mask & (1 << i)) ? ImGuiTreeSpriteFlags_Selected : 0);
                    if (i < 3)
                    {
                        // Sprite
                        bool Sprite_open = ImGui::TreeSpriteEx((void*)(intptr_t)i, Sprite_flags, "Selectable Sprite %d", i);
                    ...
                    {
                        // Leaf: The only reason we have a TreeSprite at all is to allow selection of the leaf. Otherwise we can use BulletText() or TreeAdvanceToLabelPos()+Text().
                        ImGui::TreeSpriteEx((void*)(intptr_t)i, Sprite_flags | ImGuiTreeSpriteFlags_Leaf | ImGuiTreeSpriteFlags_NoTreePushOnOpen, "Selectable Leaf %d", i);
                        if (ImGui::IsItemClicked()) 
                            Sprite_clicked = i;
                    }
                }
                if (Sprite_clicked != -1)
                {
                    ...
                    if (ImGui::GetIO().KeyCtrl)
                        selection_mask ^= (1 << Sprite_clicked);          // CTRL+click to toggle
                    else //if (!(selection_mask & (1 << Sprite_clicked))) // Depending on selection behavior you want, this commented bit preserve selection when clicking on item that is part of the selection
                        selection_mask = (1 << Sprite_clicked);           // Click to single-select
                }
                ImGui::PopStyleVar();
                if (align_label_with_current_x_position)
                    ImGui::Indent(ImGui::GetTreeSpriteToLabelSpacing());
                ImGui::TreePop();
            }
            ImGui::TreePop();
        }

        if (ImGui::TreeSprite("Collapsing Headers"))
        {
        ...
        if (ImGui::TreeSprite("Colored Text"))
        {
        ...
        if (ImGui::TreeSprite("Word Wrapping"))
        {
        ...
        if (ImGui::TreeSprite("UTF-8 Text"))
        {
        ...
        if (ImGui::TreeSprite("Selectables"))
        {
            if (ImGui::TreeSprite("Basic"))
            {
            ...
            if (ImGui::TreeSprite("Rendering more text into the same block"))
            {
            ...
            if (ImGui::TreeSprite("In columns"))
            {
        ...
        if (ImGui::TreeSprite("Filtered Text Input"))
        {
        ...
        if (ImGui::TreeSprite("Multi-line Text Input"))
        {
        ...
        if (ImGui::TreeSprite("Range Widgets"))
        {
            ImGui::Unindent();
        ...
        if (ImGui::TreeSprite("Multi-component Widgets"))
        {
            ImGui::Unindent();
        ...
        if (ImGui::TreeSprite("Vertical Sliders"))
        {
            ImGui::Unindent();
    ...
    {
        if (ImGui::TreeSprite("Child regions"))
        {
        ...
        if (ImGui::TreeSprite("Widgets Width"))
        {
            static float f = 0.0f;
        ...
        if (ImGui::TreeSprite("Basic Horizontal Layout"))
        {
        ...
        if (ImGui::TreeSprite("Text Baseline Alignment"))
        {
            ...
            if (ImGui::TreeSprite("Sprite##1")) { for (int i = 0; i < 6; i++) ImGui::BulletText("Item %d..", i); ImGui::TreePop(); }    // Dummy tree data

            ImGui::AlignFirstTextHeightToWidgets();         // Vertically align text Sprite a bit lower so it'll be vertically centered with upcoming widget. Otherwise you can use SmallButton (smaller fit).
            bool Sprite_open = ImGui::TreeSprite("Sprite##2");  // Common mistake to avoid: if we want to SameLine after TreeSprite we need to do it before we add child content.
            ImGui::SameLine(0.0f, spacing); ImGui::Button("Button##2");
            if (Sprite_open) { for (int i = 0; i < 6; i++) ImGui::BulletText("Item %d..", i); ImGui::TreePop(); }   // Dummy tree data
        ...
        if (ImGui::TreeSprite("Horizontal Scrolling"))
        {
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        ...
        if (ImGui::TreeSprite("Context menus"))
        {
    ...
    if (ImGui::CollapsingHeader("Columns"))
    {
        ...
        */
        ...
        if (ImGui::TreeSprite("Mixed items"))
        {
        ...
        bool Sprite_open = ImGui::TreeSprite("Tree within single cell");
        ImGui::SameLine(); ShowHelpMarker("NB: Tree Sprite must be poped before ending the cell.\nThere's no storage of state per-cell.");
        if (Sprite_open)
        {
            ImGui::Columns(2, "tree items");
            ImGui::Separator();
            if (ImGui::TreeSprite("Hello")) { ImGui::BulletText("Sailor"); ImGui::TreePop(); } ImGui::NextColumn();
            if (ImGui::TreeSprite("Bonjour")) { ImGui::BulletText("Marin"); ImGui::TreePop(); } ImGui::NextColumn();
            ImGui::Columns(1);
            ImGui::Separator();
            ImGui::TreePop();
        }
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
    {
        ...
        if (ImGui::TreeSprite("Focus from code"))
        {
        ...
        if (ImGui::TreeSprite("Keyboard & Mouse State"))
        {
        ...
        if (ImGui::TreeSprite("Mouse cursors"))
        {
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::TreeSprite("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
    {
        ImGui::SameLine(); ShowHelpMarker("Tip: Load fonts with io.Fonts->AddFontFromFileTTF()\nbefore calling io.Fonts->GetTex* functions.");
        ImFontAtlas* atlas = ImGui::GetIO().Fonts;
        if (ImGui::TreeSprite("Atlas texture", "Atlas texture (%dx%d pixels)", atlas->TexWidth, atlas->TexHeight))
        {
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++)
        {
            ...
            if (ImGui::TreeSprite("Details"))
            {
                ...
                if (ImGui::TreeSprite("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                {
                    ...
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                        ...
                        if (count > 0 && ImGui::TreeSprite((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base+255, count, count > 1 ? "glyphs" : "glyph"))
                        {
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    struct funcs
    {
        static void ShowDummyObject(const char* prefix, int uid)
        {
            ...
            ImGui::AlignFirstTextHeightToWidgets();  // Text and Tree Sprites are less high than regular widgets, here we add vertical spacing to make the tree lines equal high.
            bool Sprite_open = ImGui::TreeSprite("Object", "%s_%u", prefix, uid);
            ImGui::NextColumn();
            ImGui::AlignFirstTextHeightToWidgets();
