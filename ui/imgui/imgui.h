...
typedef int ImGuiTreeSpriteFlags;     // flags for TreeSprite*(), Collapsing*() // enum ImGuiTreeSpriteFlags_
...
namespace ImGui
{
    ...
    IMGUI_API void          Bullet();                                                               // draw a small circle and keep the cursor on the same line. advance cursor x position by GetTreeSpriteToLabelSpacing(), same distance that TreeSprite() uses
    ...
    // Widgets: Trees
    IMGUI_API bool          TreeSprite(const char* label);                                            // if returning 'true' the Sprite is open and the tree id is pushed into the id stack. user is responsible for calling TreePop().
    IMGUI_API bool          TreeSprite(const char* str_id, const char* fmt, ...) IM_PRINTFARGS(2);    // read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeSprite() you can use Bullet().
    IMGUI_API bool          TreeSprite(const void* ptr_id, const char* fmt, ...) IM_PRINTFARGS(2);    // "
    IMGUI_API bool          TreeSpriteV(const char* str_id, const char* fmt, va_list args);           // "
    IMGUI_API bool          TreeSpriteV(const void* ptr_id, const char* fmt, va_list args);           // "
    IMGUI_API bool          TreeSpriteEx(const char* label, ImGuiTreeSpriteFlags flags = 0);
    IMGUI_API bool          TreeSpriteEx(const char* str_id, ImGuiTreeSpriteFlags flags, const char* fmt, ...) IM_PRINTFARGS(3);
    IMGUI_API bool          TreeSpriteEx(const void* ptr_id, ImGuiTreeSpriteFlags flags, const char* fmt, ...) IM_PRINTFARGS(3);
    IMGUI_API bool          TreeSpriteExV(const char* str_id, ImGuiTreeSpriteFlags flags, const char* fmt, va_list args);
    IMGUI_API bool          TreeSpriteExV(const void* ptr_id, ImGuiTreeSpriteFlags flags, const char* fmt, va_list args);
    IMGUI_API void          TreePush(const char* str_id = NULL);                                    // ~ Indent()+PushId(). Already called by TreeSprite() when returning true, but you can call Push/Pop yourself for layout purpose
    IMGUI_API void          TreePush(const void* ptr_id = NULL);                                    // "
    IMGUI_API void          TreePop();                                                              // ~ Unindent()+PopId()
    IMGUI_API void          TreeAdvanceToLabelPos();                                                // advance cursor x position by GetTreeSpriteToLabelSpacing()
    IMGUI_API float         GetTreeSpriteToLabelSpacing();                                            // horizontal distance preceding label when using TreeSprite*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeSprite
    IMGUI_API void          SetNextTreeSpriteOpen(bool is_open, ImGuiSetCond cond = 0);               // set next TreeSprite/CollapsingHeader open state.
    IMGUI_API bool          CollapsingHeader(const char* label, ImGuiTreeSpriteFlags flags = 0);      // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
    IMGUI_API bool          CollapsingHeader(const char* label, bool* p_open, ImGuiTreeSpriteFlags flags = 0); // when 'p_open' isn't NULL, display an additional small close button on upper right of the header
    ...
    // Logging: all text output from interface is redirected to tty/file/clipboard. By default, tree Sprites are automatically opened during logging.
    ...
    IMGUI_API bool          IsItemClicked(int mouse_button = 0);                                // was the last item clicked? (e.g. button/Sprite just clicked on)
    ...
    static inline bool      CollapsingHeader(const char* label, const char* str_id, bool framed = true, bool default_open = false) { (void)str_id; (void)framed; ImGuiTreeSpriteFlags default_open_flags = 1<<5; return CollapsingHeader(label, (default_open ? default_open_flags : 0)); } // OBSOLETE 1.49+
...
// Flags for ImGui::TreeSpriteEx(), ImGui::CollapsingHeader*()
enum ImGuiTreeSpriteFlags_
{
    ImGuiTreeSpriteFlags_Selected             = 1 << 0,   // Draw as selected
    ImGuiTreeSpriteFlags_Framed               = 1 << 1,   // Full colored frame (e.g. for CollapsingHeader)
    ImGuiTreeSpriteFlags_AllowOverlapMode     = 1 << 2,   // Hit testing to allow subsequent widgets to overlap this one
    ImGuiTreeSpriteFlags_NoTreePushOnOpen     = 1 << 3,   // Don't do a TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor pushing on ID stack
    ImGuiTreeSpriteFlags_NoAutoOpenOnLog      = 1 << 4,   // Don't automatically and temporarily open Sprite when Logging is active (by default logging will automatically open tree Sprites)
    ImGuiTreeSpriteFlags_DefaultOpen          = 1 << 5,   // Default Sprite to be open
    ImGuiTreeSpriteFlags_OpenOnDoubleClick    = 1 << 6,   // Need double-click to open Sprite
    ImGuiTreeSpriteFlags_OpenOnArrow          = 1 << 7,   // Only open when clicking on the arrow part. If ImGuiTreeSpriteFlags_OpenOnDoubleClick is also set, single-click arrow or double-click all box to open.
    ImGuiTreeSpriteFlags_Leaf                 = 1 << 8,   // No collapsing, no arrow (use as a convenience for leaf Sprites). 
    ImGuiTreeSpriteFlags_Bullet               = 1 << 9,   // Display a bullet instead of arrow
    //ImGuITreeSpriteFlags_SpanAllAvailWidth  = 1 << 10,  // FIXME: TODO: Extend hit box horizontally even if not framed
    //ImGuiTreeSpriteFlags_NoScrollOnOpen     = 1 << 11,  // FIXME: TODO: Disable automatic scroll on TreePop() if Sprite got just open and contents is not visible
    ImGuiTreeSpriteFlags_CollapsingHeader     = ImGuiTreeSpriteFlags_Framed | ImGuiTreeSpriteFlags_NoAutoOpenOnLog
};
...
// Condition flags for ImGui::SetWindow***(), SetNextWindow***(), SetNextTreeSprite***() functions
...
struct ImGuiStyle
{
    ...
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a tree Sprite. Generally == (FontSize + FramePadding.x*2).
...
// - You want to manipulate the open/close state of a particular sub-tree in your interface (tree Sprite uses Int 0/1 to store their state).
...
struct ImGuiStorage
{
    ...
    // Use on your own storage if you know only integer are being stored (open/close all tree Sprites)
